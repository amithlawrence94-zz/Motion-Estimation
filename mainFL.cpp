...
int main( int argc, char** argv )
{
    ...
    cv::Mat bgr_frame1, frame_gray1, imag_1, imag_2, img_keypoints,bgr_frame2, frame_gray2,gray2, img_keypoints2, bgr_frame3,frame_gray3,img_keypoints3, img_1, img_2;
    ...
    //cv::cvtColor( img_1, imag_1, cv::COLOR_GRAY2BGR);

    Descriptor->compute(bgr_frame1, keypoints_1, descriptors_1);// computing descriptors for the corresponding keypoints using FREAK descriptor
    ...
    //cv::cvtColor( img_2, imag_2, cv::COLOR_GRAY2BGR);

    Descriptor-> compute(bgr_frame2, keypoints_2, descriptors_2); // computing descriptors for keypoints_2 using FREAK
    ...
    writer.open("outputFL.mp4", 0x7634706d, fps, size );// output video path and type
    ...
    for(;;) // infinite loop
    {
        ...
        for( signed int i = 0; i < descriptors_1.rows; i+=1 )
...
*/
        ...
        for(size_t i=0; i<points2f2.size(); i++)
        {
        	if ( i != 0 )
			{
				...
				//line(imgLines,points2f1[i],points2f2[i],color[i],1);
			}
        ...
        Detector->detect(img_2, keypoints_2);// detecting keypoints for frame_gray2 using SURF algorithm

        //cv::cvtColor( img_2, imag_2, cv::COLOR_GRAY2BGR);

        Descriptor->compute(bgr_frame2, keypoints_2, descriptors_2);//computing descriptors for keypoints_2
